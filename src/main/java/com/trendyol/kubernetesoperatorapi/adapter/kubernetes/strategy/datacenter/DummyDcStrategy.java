...
@Slf4j
@Service
public class DummyDcStrategy implements KubernetesClientStrategy {

    private volatile DataCenterSettings dataCenterSettings = null;

    @Override
    public String getDataCenter() {
        return DataCenter.DUMMY.getValue();
    }

    @Override
    public DataCenterSettings getClient() {
        String kubeConfigPath = "/etc/kube/dummy/DUMMY";

        if (dataCenterSettings == null) {
            synchronized (DummyDcStrategy.class) {
                if (dataCenterSettings == null) {
                    try {
